...
#based on the work of https://github.com/nkostopoulos/StochasticsLabPublic/tree/master/lab8

import json

problematic1 = open("data/problematic_valid.txt", "w")
problematic2 = open("data/problematic_invalid.txt", "w")
...
def calculate_probabilities(dataset):
    ...
    for index in range(0, 7):
        for key in stats[index]:
            ...
            stats[index][key] *= 1000
            stats[index][key] = int(stats[index][key])
            if (stats[index][key] == 0):
                stats[index][key] = 1
    return stats
...
#when reading probs from json files
def find_prob2(prefix, stats, fd):
    tl, td, mns, tc, mcs, tv, mvs = handle_name(prefix)
    try:
        prob = stats['0'][str(tl)] * stats['1'][str(td)] * stats['2'][str(mns)] * stats['3'][str(tc)] * stats['4'][str(mcs)] * stats['5'][str(tv)] * stats['6'][str(mvs)]
    except:
        prob = 0
        fd.write(prefix + "\n")
    return prob
...
def apply_on_test_set(test_set, category, valid_stats, invalid_stats, fd):
    ...
    for prefix in test_set:
        ...
        if valid_prob != 0 or invalid_prob != 0:
            names_processed += 1
...
def dump_stats_to_file(valids, invalids):
    valid = open("valid_stats.json", "w")
    invalid = open("invalid_stats.json", "w")
    json.dump(valids, valid)
    json.dump(invalids, invalid)
    valid.close()
    invalid.close()
...
if __name__ == "__main__":
    ...
    valid_names_training = load_file("data/valid_training.txt")
    # Load valid names test set
    valid_names_test = load_file("data/valid_test.txt")
    # Load invalid names training set
    invalid_names_training = load_file("data/invalid_training.txt")
    # Load invalid names test set
    invalid_names_test = load_file("data/invalid_test.txt") 

    valid_stats = calculate_probabilities(valid_names_training)
    invalid_stats = calculate_probabilities(invalid_names_training)

    #dump_stats_to_file(valid_stats, invalid_stats)
    ...
    print("valid processed: " + str(valid_names_processed))
    print("invalid processed: " + str(invalid_names_processed))